...
class BackendResponse extends React.Component {
    constructor() {
        super()
        this.state = {
    ...
    MainMaterials(baseinfo, matinfo) {
    ...
    render() {
        if (Object.entries(this.props.recipeData).length === 0) {
            return <div>Database may be down!</div>
        }
        const mainrecipe = this.MainRecipe(this.props.recipeData.MainRecipe)
        const mainmaterials = this.MainMaterials(this.props.recipeData.MainRecipe, this.props.recipeData.InnerRecipes)
        return (<div>
            {mainrecipe}
            {mainmaterials}
        </div>)
